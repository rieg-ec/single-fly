...
const pagesBase=process.env.FLY_BASE_PATH||'/single-fly/';
for(const base of ['/', pagesBase]){
...
for(const match of html.matchAll(/(?:src|href)="([^"]+)"/g)){
 assert.ok(match[1].startsWith(pagesBase),match[1]);assert.ok(existsSync('dist-pages/'+match[1].slice(pagesBase.length)));
}
const bundle=JSON.parse(readFileSync('public/source/app-files.json'));
for(const path of ['.github/workflows/pages.yml','lib/asset-path.ts','vite.config.ts','index.html','main.tsx'])assert.ok(bundle.some(f=>f.path===path));
